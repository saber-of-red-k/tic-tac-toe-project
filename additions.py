...
def receive_input_column():
    while True:
        ...
        if inp == '1' or inp == '2' or inp == '3':              #same as row check
            column = int(inp)
            return column
        else: continue
...
def next_turn_win(board,operator):              #board + operator (X or O)
    for lstIdx, lst in enumerate(board):
        if lst.count(operator) == 2:             #check to see if 2 elements match in rows
    ...
    for lstIdx, lst in enumerate(reversed_board):                   #same check
        if lst.count(operator) == 2:
            index = lstIdx
            for idx, item in enumerate(lst):
                if reversed_board[lstIdx][idx] == '-':
                    print(f'Next turn win if you choose row {idx + 1}, column {index + 1}')         #row and column swapped

    if board[0][0] == board[1][1] == operator:
        print(f'Next turn win if you choose row {2 + 1}, column {2 + 1}')
    elif board[0][0] == board[2][2] == operator:
        print(f'Next turn win if you choose row {1 + 1}, column {1 + 1}')                           #diagonals check 1
    elif board[1][1] == board[2][2] == operator:
        print(f'Next turn win if you choose row {0 + 1}, column {0 + 1}')

    if board[0][2] == board[1][1] == operator:
        print(f'Next turn win if you choose row {2 + 1}, column {0 + 1}')
    elif board[0][2] == board[2][0] == operator:
        print(f'Next turn win if you choose row {1 + 1}, column {1 + 1}')                           #diagonals check 2
    elif board[1][1] == board[2][0] == operator:
        print(f'Next turn win if you choose row {0 + 1}, column {2 + 1}')
...
while True:
    if mode == 'pc':
        ...
        if current_player_symbol == 'X':
            ...
            row = random.randint(0, 2)              #if we are in pc mode, instead of input we choose random numbers until it hits empty space
            column = random.randint(0, 2)

    elif mode == '2pc':
        row = random.randint(0,2)                   #in 2pc mode we go full random
        column = random.randint(0, 2)
    else:
        row = receive_input_row() - 1  # using -1 so the player can type normal numbers for row and column
        column = receive_input_column() - 1  # ex. 3 instead of 2 and 1 instead of 0
